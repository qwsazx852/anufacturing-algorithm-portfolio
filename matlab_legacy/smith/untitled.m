...
%% 適應值計算
[fxm]=fitness(fxm_ans,T1,T22,PP);%各別碳足跡與利潤之R-metric，fxm_ans=1x100cell，length=零件數
%每個個體之歷史最佳適應值
c=[];
%% 擁擠距離
% in=[];
% for run =1:length(fxm_ans)
%     in=[in;fxm_ans{run}];
% end
%  [a,b]=cown(in,fxm);
a=fxm_ans;
...
for i1 = 1:ger
   i1 
%% 拆裝
    fxm_ans2=pso_samedifference(fxm_ans,a1,b1,n,Or,c1,c2);%機制：同中求異柏拉圖解版本
%     fxm_ans2= ppx(n,fxm_ans,crossover,PP);%機制：ppx交配法 
%     fxm_ans2=PSO_ALBP01(PP,n,Or);%機制：隨機產生族群
     [score2]=fitness (fxm_ans2,T1,T22,PP);%適應值         
    [fxm,fxm_ans]=pso_anser (fxm_ans,fxm,fxm_ans2,score2,w);
%     if i1>=2
%         for ii=1:length(A(:,1))
%             fxm_ans{PP+ii}=a1(ii,:);
%         end
...
%          while  B2<=max(D)
%              HV=[]
%              a= LBb222(n,fxm_ans,Or,mutation); %突變TAX
%              [fxm]=fitness(a,T1,T22,PP);
%              for i =1:length(a)
%                  hv = hypervolumetwo([a(i),fxm(i)],samples);
%                  HV=[HV;hv];
%              end
%              B2=max(HV)
%              max(D)
...
%      end
%  end
%% 鄰域搜尋2%% 保留柏拉圖解並產生新初始族群

pp=length(a)-length(A(:,1));
if i1>Ns
 [fxm_ans] = ppx(n,fxm_ans,crossover,pp);%交配選拆後
 for i =1:length(A(:,1))
     fxm_ans{pp+i}=a1(i,:);
 end
 [fxm]=fitness(fxm_ans,T1,T22,PP);
    % %% 適應值計算
    % [score2,fym2,fym_ans2,R2]=fitness (fxm_ans2,T1,T22,PP);%適應值         
    % [fxm,fym, fzm,fxm_ans, fym_ans,fzm_ans,R,R3]=pso_anser (fym_ans,fxm_ans, fzm_ans,fxm,fym,fzm,fxm_ans2,score2,w,R2,fym2,fym_ans2,R3,R);
...
ylabel('適應值')
rans = Rmetric(A)
best=a1(find(rans==min(rans),1),:)
SS= taxlbss(best,samples,T1,T22);
psoans=[]
for iii =1:length(A)
beststop=LBssR(a1(iii,:),T1,T22,samples)
