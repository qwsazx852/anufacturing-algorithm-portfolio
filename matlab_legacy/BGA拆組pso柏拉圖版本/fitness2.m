function SS = fitness2(s,T1,T22); %計算適應值(f2碳足跡)
%%
%% 新版
%% 懲罰值式決定停止點
%%  new
...
% s=[ 12    11     9     1     7     6     5    13    15     8     2     4     3    17    10    18    16    14]
% asb=[18];
% a=[];
% add=0;
% ans=[];
...
%     if i==1
%         set=reuse;
%         for i2=1:length(reuse)
%             rt(reuse(i2))=1;
%         end
%     elseif i==2
%         set=recycle;
%         for i2=1:length(recycle)
%             rt(recycle(i2))=-0.5;
%         end
%     elseif i==3
%         set=Remanufacturing;
%         for i2=1:length(Remanufacturing)
%             rt(Remanufacturing(i2))=-0.3;
%         end
%     else
%         set=t;
%         for i2=1:length(t)
...
A=sum(rt(s(1:i)));
B=sum(rt(s(i+1:end)));
stopselectadd=[stopselectadd;A,B,A./B];
end
stoppoint=find(stopselectadd(:,1)==max(stopselectadd(:,1)))+1;%+1因為頭尾不計算
